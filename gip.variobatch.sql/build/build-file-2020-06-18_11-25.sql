...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'V' AND name = 'ProdOrderOutwardsView')
	BEGIN
		DROP  View dbo.[ProdOrderOutwardsView]
	END
GO

CREATE VIEW [dbo].[ProdOrderOutwardsView]
	AS 
...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'V' AND name = 'ProdOrderConnectionsView')
	BEGIN
