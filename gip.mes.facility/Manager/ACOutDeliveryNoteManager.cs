...
namespace gip.mes.facility
{
    ...
    public partial class ACOutDeliveryNoteManager : PARole
    {
        ...
        public virtual Msg UnassignContractOutOrderPos(OutOrderPos currentReleasePos, DatabaseApp dbApp)
        {
            ...
            parentOutOrderPos.OutOrderPos_ParentOutOrderPos.AutoLoad(parentOutOrderPos.OutOrderPos_ParentOutOrderPosReference, parentOutOrderPos);
            currentReleasePos.DeleteACObject(dbApp, false);

            // 3. Korrigiere Status der Parent-Bestellposition
            var queryChildsSubPos = parentOutOrderPos.OutOrderPos_ParentOutOrderPos.Where(c => c.EntityState != EntityState.Deleted);
        ...
        public List<FacilityPreBooking> CancelFacilityPreBooking(ACComponent facilityManager, DatabaseApp dbApp, DeliveryNotePos deliveryNotePos)
        {
            ...
            if (deliveryNotePos.OutOrderPos == null || deliveryNotePos.OutOrderPos.MDOutOrderPosState.OutOrderPosState == MDOutOrderPosState.OutOrderPosStates.Cancelled)
                return null;
            if (deliveryNotePos.OutOrderPos.EntityState != EntityState.Added)
            {
                deliveryNotePos.OutOrderPos.FacilityBooking_OutOrderPos.AutoLoad(deliveryNotePos.OutOrderPos.FacilityBooking_OutOrderPosReference, deliveryNotePos);
                deliveryNotePos.OutOrderPos.FacilityPreBooking_OutOrderPos.AutoLoad(deliveryNotePos.OutOrderPos.FacilityPreBooking_OutOrderPosReference, deliveryNotePos);
            }
            else
                return null;
        ...
        */
        ...
        public Msg ValidateCurrencyAndExchangeRate(OutOrder outOrder, DateTime invoiceDate)
        {
            ...
            {
                // Issuer address for invoice not set in order.
                return new Msg
                {
                    Source = GetACUrl(),
                    MessageLevel = eMsgLevel.Error,
                    ACIdentifier = "ValidateCurrencyAndExchangeRate(0)",
                    Message = Root.Environment.TranslateMessage(this, "Error50441")
                };
            }
            if (outOrder.IssuerCompanyAddress.MDCountry == null)
            {
                // Country was not assigned to the issuers address.
                return new Msg
                {
                    Source = GetACUrl(),
                    MessageLevel = eMsgLevel.Error,
                    ACIdentifier = "ValidateCurrencyAndExchangeRate(1)",
                    Message = Root.Environment.TranslateMessage(this, "Error50442")
                };
            }
            if (outOrder.IssuerCompanyAddress.MDCountry.MDCurrency == null)
            {
                // No currency was assigned to the issuers country.
                return new Msg
                {
                    Source = GetACUrl(),
                    MessageLevel = eMsgLevel.Error,
                    ACIdentifier = "ValidateCurrencyAndExchangeRate(2)",
                    Message = Root.Environment.TranslateMessage(this, "Error50443")
                };
            }
            if (!outOrder.MDCurrencyID.HasValue)
            {
                // The order has no currency.
                return new Msg
                {
                    Source = GetACUrl(),
                    MessageLevel = eMsgLevel.Error,
                    ACIdentifier = "ValidateCurrencyAndExchangeRate(3)",
                    Message = Root.Environment.TranslateMessage(this, "Error50444")
                };
            }
            ...
            if (exchangeRate == null)
            {
                ...
                return new Msg
                {
                    Source = GetACUrl(),
                    MessageLevel = eMsgLevel.Error,
                    ACIdentifier = "ValidateCurrencyAndExchangeRate(4)",
                    Message = Root.Environment.TranslateMessage(this, "Error50445")
                };
            }
            return null;
        }
                        ...
                        case "MDUnitID":
                            {
                                ...
                                posItem.OnEntityPropertyChanged("ForeignTotalPrice");
                                posItem.OnEntityPropertyChanged("ForeignTotalPriceWithTax");
        ...
        private InvoicePos GetInvoicePos(DatabaseApp databaseApp, Invoice invoice, int nr, OutOrderPos outOrderPos)
        {
            ...
            if (invoice.MDCurrencyExchange != null)
            {
                invoicePos.ForeignPriceNet = outOrderPos.PriceNet;
                invoicePos.ForeignPriceGross = outOrderPos.PriceGross;
            }
            else
            {
