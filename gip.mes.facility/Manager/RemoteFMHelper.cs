...
";
        ...
        public List<RemotePickingInfo> GetRemotePickingInfos(DatabaseApp databaseApp, ACComponent aCComponent, string mdPickingType, string remoteConnString)
        {
            ...
            FormattableString cntQueryStringSysProc = FormattableStringFactory.Create(sql, localDatabaseName);
            List<RemotePickingInfo> remotePickingInfos = databaseApp.Database.SqlQuery<RemotePickingInfo>(cntQueryStringSysProc).ToList<RemotePickingInfo>();
                ...
                foreach (PickingPos pos in positions)
                {
                    ...
                    if (mirroredPosition != null && !mirroredPosition.FacilityBooking_PickingPos.Any())
                    {
                        ...
                        foreach (FacilityBooking fb in fbc)
                        {
                            ...
                            databaseApp.FacilityPreBooking.Add(preBooking);
                            databaseApp.ACSaveChanges();
                        }
                    }
                    else
