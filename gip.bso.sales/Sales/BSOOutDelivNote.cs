...
namespace gip.bso.sales
{
    ...
    public class BSOOutDeliveryNote : ACBSOvbNav
    {
        ...
        public IEnumerable<OutOrderPos> OutOrderPosFromPickingList
        {
            get
            {
                var query = DatabaseApp.PickingPos.Where(c => (c.Picking.PickingTypeIndex == (short)GlobalApp.PickingType.IssueVehicle || c.Picking.PickingTypeIndex == (short)GlobalApp.PickingType.Issue)
                                                          && c.OutOrderPos != null
        ...
        [ACPropertyList(621, "FacilityLots")]
        public IEnumerable<FacilityLot> BookableFacilityLots
        {
            get
            {
                if ((CurrentDeliveryNotePos == null) || (CurrentDeliveryNotePos.OutOrderPos == null) || (CurrentACMethodBooking == null))
                    return null;
                ...
                {
                    var query2 = FacilityBookingList.Where(c => c.OutwardFacilityLot != null).Select(c => c.OutwardFacilityLot).Distinct();
                ...
                var queryFromDB = DatabaseApp.FacilityLot.Where(c => c.Material != null && c.MaterialID == CurrentDeliveryNotePos.OutOrderPos.MaterialID);
                if (bookableFacilityLots == null)
                    bookableFacilityLots = bookableFacilityLots.ToList();
                else
                {
                ...
                case "CurrentACMethodBooking\\OutwardFacilityLot":
                    {
                        ...
                        {
                            if (!CurrentDeliveryNotePos.OutOrderPos.Material.IsLotManaged)
