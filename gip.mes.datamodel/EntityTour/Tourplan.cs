...
namespace gip.mes.datamodel
{
    ...
    [ACPropertyEntity(3, "VehicleFacility", "en{'Vehicle'}de{'Fahrzeug'}", Const.ContextDatabase + "\\" + Facility.ClassName + Const.DBSetAsEnumerablePostfix, "", true)]
    [ACPropertyEntity(4, "TrailerFacility", "en{'Trailer'}de{'Anhänger'}", Const.ContextDatabase + "\\" + Facility.ClassName + Const.DBSetAsEnumerablePostfix, "", true)]
    [ACPropertyEntity(5, Company.ClassName, "en{'Company'}de{'Firma'}", Const.ContextDatabase + "\\" + Company.ClassName + Const.DBSetAsEnumerablePostfix, "", true)]
    [ACPropertyEntity(6, MDTour.ClassName, "en{'Tour'}de{'Tour'}", Const.ContextDatabase + "\\" + MDTour.ClassName + Const.DBSetAsEnumerablePostfix, "", true)]
    ...
    [ACPropertyEntity(10, "VisitorCardFacilityCharge", "en{'Transponder Card'}de{'Transponderkarte'}", Const.ContextDatabase + "\\" + FacilityCharge.ClassName + Const.DBSetAsEnumerablePostfix, "", true)]
    ...
    [ACPropertyEntity(21, MDTourplanState.ClassName, "en{'Picking Status'}de{'Kommissionierstatus'}", Const.ContextDatabase + "\\" + MDTourplanState.ClassName + Const.DBSetAsEnumerablePostfix, "", true)]
    ...
    public partial class Tourplan : IACConfigStore
    {
        ...
        private SafeList<IACConfig> ACConfigListCache
        {
            get
            {
                ...
                if (TourplanConfig_Tourplan_IsLoaded)
                {
                    TourplanConfig_Tourplan.AutoLoad(TourplanConfig_TourplanReference, this);
                }
        ...
        public bool ValidateConfigurationEntriesWithDB(ConfigEntriesValidationMode mode = ConfigEntriesValidationMode.AnyCheck)
        {
            ...
            {
                var query = database.TourplanConfig.Where(c => c.TourplanID == this.TourplanID);
                if (mode == ConfigEntriesValidationMode.AnyCheck)
                {
                    if (query.Any())
                        return false;
