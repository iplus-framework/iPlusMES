...
begin
	...
	set Pos.ActualQuantityUOM = 
		isnull((select sum(isnull(Fbc.OutwardQuantityUOM, 0)) + sum(isnull(Fbc.InwardQuantityUOM, 0))   from FacilityBookingCharge Fbc inner join FacilityBooking Fb on Fb.FacilityBookingID = Fbc.FacilityBookingID where Fb.ProdOrderPartslistPosID = Pos.ProdOrderPartslistPosID and fb.FacilityBookingTypeIndex <> 106),0)	
